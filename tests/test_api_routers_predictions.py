...
@patch("api.app.routers.predictions.predict_commune")
def test_predict_by_commune_success(mock_predict_commune):
    ...
    response = client.get("/predictions/commune/35238")
...
def test_predict_by_commune_not_found(mock_predict_commune):
    ...
    response = client.get("/predictions/commune/99999")
    assert response.status_code == 404

@patch("api.app.routers.predictions.predict_logement")
def test_predict_by_logement_success(mock_predict_logement):
    ...
    response = client.post("/predictions/logement", json={
        "code_insee": "35238",
        "surface_m2": 75.0,
        "nb_pieces": 4,
        "nb_logements": 1
...
@patch("api.app.routers.predictions.predict_batch_csv")
...
def test_predict_batch_success(mock_read_csv, mock_predict_batch):
    ...
    response = client.post("/predictions/batch", files={"file": ("test.csv", content)})
    assert response.status_code == 200
    assert len(response.json()) == 1

def test_predict_batch_invalid_file():
    response = client.post("/predictions/batch", files={"file": ("test.txt", "not a csv")})
